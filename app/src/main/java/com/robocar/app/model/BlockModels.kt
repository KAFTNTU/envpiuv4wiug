...
enum class BlockCategory(val label: String, val color: Long) {
    CAR       ("Машинка",    0xFF4C97FFL),
    CONTROL   ("Керування",  0xFFFFBF00L),
    SENSORS   ("Сенсори",    0xFF00897BL),
    MATH      ("Математика", 0xFF9966FFL),
    STATE     ("Стан",       0xFF8E24AAL),
    SMART     ("Контроль",   0xFFE65100L),
    LOGIC     ("Логіка",     0xFF5CB1D6L),
    VARIABLES ("Змінні",     0xFFFF8C1AL),
    LOOPS     ("Цикли",      0xFF5BA55BL),
}
...
enum class BlockType(
    ...
    STATE_PREV      ("Повернутись у попер.",   "", BlockCategory.STATE,   0xFF6A1B9AL),
    STATE_IF        ("Якщо стан =",             "", BlockCategory.STATE,   0xFF6A1B9AL, hasSub = true, hasSub2 = true),

    // === ⚡ РОЗУМНІ УМОВИ ===
    WAIT_UNTIL_TRUE_FOR("Чекати поки умова",      "", BlockCategory.SMART, 0xFFBF360CL),
    TIMEOUT_DO_UNTIL("Робити до умови",          "",  BlockCategory.SMART, 0xFFBF360CL, hasSub = true),
    COOLDOWN_DO     ("Не частіше ніж N с",      "",  BlockCategory.SMART, 0xFFBF360CL, hasSub = true),
    LATCH_SET       ("Прапор встановити",       "",  BlockCategory.SMART, 0xFFBF360CL),
    MOTOR_4         ("4 Мотори (ABCD)",          "", BlockCategory.CAR,     0xFF4527A0L),
    STATE_GET       ("Поточний стан",             "", BlockCategory.STATE,   0xFF6A1B9AL),
    STATE_TIME_S    ("Час у стані (с)",           "", BlockCategory.STATE,   0xFF6A1B9AL),
    STATE_ENTER_COUNT("Скільки разів у стан",     "", BlockCategory.STATE,   0xFF6A1B9AL),
    LATCH_GET       ("Прапор встановлено?",       "", BlockCategory.SMART,   0xFFBF360CL),
    IF_TRUE_FOR     ("Якщо умова тримається",     "", BlockCategory.SMART,   0xFFBF360CL, hasSub = true, hasSub2 = true),
    EDGE_DETECT     ("Сигнал став активним (0→1)","", BlockCategory.SMART,   0xFFBF360CL),
    SCHMITT_TRIGGER ("Тригер Шмітта",             "", BlockCategory.SMART,   0xFFBF360CL),
    ...
    SENSOR_GET    ("Значення сенсора",     "", BlockCategory.SENSORS, 0xFF00897BL, hasNext = false, hasPrev = false),
