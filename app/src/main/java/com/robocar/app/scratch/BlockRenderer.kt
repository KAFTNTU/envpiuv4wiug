...
// ───────────────────────────────────────────────────────────────
// LABEL + PARAM BUBBLES — як у Blockly
// Рендер: label | [bubble val] | [bubble val] ...
// Bubble: білий/темний овал з текстом (NumberInput, Dropdown)
// ───────────────────────────────────────────────────────────────
private fun DrawScope.drawWsBlockLabel(
    ...
    val D         = BlockDimensions
    val hatOff    = if (!block.type.hasPrev) D.HAT_EXTRA * 0.3f else 0f
    val bodyH     = if (!block.type.hasPrev) bh - D.HAT_EXTRA else bh
    val cy        = by + hatOff + bodyH / 2f
    val colD      = blockColorDark(block.type)

    // ── 1. Заголовок блоку ─────────────────────────────────
    val label   = blockLabel(block.type)
    val lMeas   = textMeasurer.measure(
        AnnotatedString(label),
        TextStyle(fontSize = 12.sp, fontWeight = FontWeight.Bold, color = Color.White)
    )
    val startX = bx + D.STRIPE_W + 10f
    drawText(lMeas, topLeft = Offset(startX, cy - lMeas.size.height / 2f))

    // ── 2. Param bubbles ───────────────────────────────────
    // Розміщуємо inline праворуч від заголовку
    // Кожен param → овальний bubble (як у Blockly)
    val BUBBLE_H    = bodyH * 0.56f            // висота bubble ~56% висоти тіла
    val BUBBLE_R    = BUBBLE_H / 2f            // радіус округлення
    val BUBBLE_PAD  = 8f                        // горизонтальний padding тексту в bubble
    val GAP         = 6f                        // відступ між елементами

    // Відступ для inline-параметрів
    var cx = startX + lMeas.size.width + GAP

    val inlineParams = block.params.filterNot { it is BlockParam.SubProgram }

    for (param in inlineParams) {
        val valStr = when (param) {
            is BlockParam.NumberInput   -> {
                val v = param.value
                if (v == v.toLong().toFloat()) v.toInt().toString() else "%.1f".format(v)
            }
            is BlockParam.DropdownInput ->
                (param.options.find { it.second == param.selected }?.first ?: "?").take(8) + " ▼"
            is BlockParam.TextInput     -> param.value.take(10)
            else                        -> continue
        }

        // Додатковий label параметру (якщо є, напр "L", "R", "за (сек)")
        val pLabel = when (param) {
            is BlockParam.NumberInput   -> param.label.take(4)
            is BlockParam.DropdownInput -> ""
            is BlockParam.TextInput     -> param.label.take(4)
            else -> ""
        }

        // Малюємо label param (напр "L")
        if (pLabel.isNotEmpty() && pLabel.length <= 3) {
            val plMeas = textMeasurer.measure(
                AnnotatedString(pLabel),
                TextStyle(fontSize = 10.sp, fontWeight = FontWeight.Bold, color = Color.White.copy(0.8f))
            )
            // Тільки якщо влазить
            if (cx + plMeas.size.width + GAP + 30f < bx + bw - 8f) {
                drawText(plMeas, topLeft = Offset(cx, cy - plMeas.size.height / 2f))
                cx += plMeas.size.width + 4f
            }
        }

        // Вимірюємо текст value
        val vMeas = textMeasurer.measure(
            AnnotatedString(valStr),
            TextStyle(fontSize = 11.sp, fontWeight = FontWeight.Bold,
                color = Color(0xFF1A1A2E))
        )

        val bubbleW = vMeas.size.width + BUBBLE_PAD * 2f

        // Перевіряємо що влазить
        if (cx + bubbleW > bx + bw - 6f) break

        // Bubble фон — білий/кремовий овал
        val isDropdown = param is BlockParam.DropdownInput
        val bubbleColor = if (isDropdown) Color(0x33FFFFFF) else Color.White

        drawRoundRect(
            color       = bubbleColor,
            topLeft     = Offset(cx, cy - BUBBLE_H / 2f),
            size        = Size(bubbleW, BUBBLE_H),
            cornerRadius = CornerRadius(BUBBLE_R, BUBBLE_R),
        )

        // Value текст у bubble
        val textColor = if (isDropdown) Color.White else Color(0xFF1A1A2E)
        val vMeas2 = textMeasurer.measure(
            AnnotatedString(valStr),
            TextStyle(fontSize = 11.sp, fontWeight = FontWeight.Bold, color = textColor)
        )
        drawText(vMeas2, topLeft = Offset(
            cx + BUBBLE_PAD,
            cy - vMeas2.size.height / 2f,
        ))
