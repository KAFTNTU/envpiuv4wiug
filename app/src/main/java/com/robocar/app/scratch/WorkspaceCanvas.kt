...
fun WorkspaceCanvas(
    ...
    // drag tracking state — collected outside canvas for update callbacks
    var onUpdate by remember { mutableStateOf<((Float, Float) -> Unit)?>(null) }
    var onEnd    by remember { mutableStateOf<((Float, Float, Float) -> Unit)?>(null) }
                ...
                if (isDragging) {
                    awaitEachGesture {
                        while (true) {
                            ...
                            ch.consume()
                            val pos = ch.position
                            onUpdate?.invoke(pos.x, pos.y)
                            if (!ch.pressed) {
                                onEnd?.invoke(pos.x, pos.y, canvasH)
                                break
                            }
                        }
                    }
